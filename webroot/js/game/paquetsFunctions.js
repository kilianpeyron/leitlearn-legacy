...
export function updateModalContent(data) {
    ...
    var selected_packet = $('#selected_packet');
    selected_packet.empty();

    if (userPackets && userPackets.length > 0) {
        $.each(userPackets, function (index, userPacket) {
            var option = $('<option>').attr('value', userPacket.id).text(userPacket.name);
            selected_packet.append(option);
        });
    }

    var creator = data.creator;
    var creator_packet = $('#creator');
    creator_packet.empty();

    var divCreator = $('<div>').addClass('packet-creator');

    var imgAvatar = $('<img>').attr({
        'src': '/img/user_profile_pic/' + creator.profile_picture,
        'class': 'avatar',
        'alt': ''
    });

    var strongName = $('<strong>').text(creator.username);

    divCreator.append(imgAvatar, strongName);
    creator_packet.append(divCreator);
}
