...
const aiRequestEvent = () => {
    $('#leitlearn_ai_submit').on("click", function () {
        $('#leitlearn_ai_submit').hide();
        loadingEvent();
    });
}
...
const loadingEvent = () => {
    for (let i = 1; i <= 5; i++) {
        const flashcard = $("<div>").addClass("flashcard");

        const flashcard_question = $("<div>").addClass("question ai-skeleton");
        flashcard.append(flashcard_question);

        const flashcard_answer = $("<div>").addClass("answer ai-skeleton");
        const flashcard_answser_content = $("<div>").addClass("content");
        flashcard_answer.append(flashcard_answser_content);
